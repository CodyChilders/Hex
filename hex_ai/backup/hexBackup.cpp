...
inline bool playerOneCheck(Graph* graph){
	...
	for(int i = 0; i < (size + 1); ++i){
		...
		for(int j = (size * (size - 1)); j < ((size * size) + 1); ++j){
...
int main(void){
	...
	while(true){
		cout << "Would you like an 11x11 or 13x13 board? ";
		cin >> size;
		cout << endl;
		if(cin.fail()){
	...
	for(int turn = 0; /*no exit condition*/; turn++){
		...
		cout << "Enter the row and column coordinate with only a space inbetween" << endl;
		//get their move
		int i, j;
		while(true){
			...
			if(player == p2){
				...
				if(pieceBoard[i][j] != 0){//ignores pie rule
					...
					int* move = ai.getMove(pieceBoard);
					i = move[0];
					j = move[1];
					delete move;
			}
